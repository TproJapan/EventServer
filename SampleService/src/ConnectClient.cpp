...
{
	writeLog(2, "constructor started. dstSocket=%d, %s %d %s\n", dstSocket, __FILENAME__, __LINE__, __func__);
	_socket = dstSocket;
	live = true;
}
...
void ConnectClient::func()
{
	...
	while (1) {
		...
		// 通信
		///////////////////////////////////
		writeLog(2, "client(%d)クライアントとの通信を開始します, %s %d %s\n", _socket, __FILENAME__, __LINE__, __func__);
		size_t stSize;
		char buf[1024];
